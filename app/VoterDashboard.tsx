...
export default function VoterDashboard() {
  ...
  const [registryStatus, setRegistryStatus] = useState<VoterRegistryRow | null>(
    null,
  );
  ...
  const [votedElectionIds, setVotedElectionIds] = useState<Set<string>>(
    new Set(),
  );
  ...
  const getEffectiveElectionStatus = (
  ...
  useEffect(() => {
    const loadSelectedElection = async () => {
      ...
      try {
        const candidateRows =
          await serviceFactory.votingService.getElectionCandidates(
            selectedElection.id,
          );
        const voterRegistry =
          getEffectiveElectionStatus(selectedElection) === "active"
            ? await serviceFactory.votingService.getMyRegistryStatus(
                selectedElection.id,
              )
            : null;
  ...
  const handleVote = async () => {
    ...
    if (!selectedElection) {
      validationError = {
        title: "No Election",
        message: "There is no active election available right now.",
      };
    } else if (getEffectiveElectionStatus(selectedElection) !== "active") {
      validationError = {
        title: "Election Closed",
        message: "Only open elections can accept votes.",
      };
      ...
      validationError = {
        title: "Not Eligible",
        message: "You are not registered as eligible for this election.",
      };
    ...
    if (!selectedElection || !selectedCandidateId) {
      console.error(
        "[Vote] Unexpected: selectedElection or selectedCandidateId is null after validation",
      );
      return;
    }
    ...
    try {
      console.log(
        "[Vote] Calling castVote for election:",
        selectedElection.id,
        "candidate:",
        selectedCandidateId,
      );
      ...
      if (votedCandidate) {
        ...
        setVoteDetails((previous) =>
          new Map(previous).set(selectedElection.id, voteReceipt),
        );
      }
      ...
      try {
        ...
        console.warn(
          "Vote succeeded but registry refresh failed:",
          refreshError,
        );
      }
      ...
      const isAlreadyVoted =
        /already\s+has\s+a\s+recorded\s+vote|already\s+voted/i.test(message);
  ...
  const fetchVoteDetails = async (
    ...
    try {
      const candidates =
        await serviceFactory.votingService.getElectionCandidates(electionId);
      let ledger: Awaited<
        ReturnType<typeof serviceFactory.auditorService.getLedger>
      > = [];
      ...
      try {
        ...
        console.warn(
          "Ledger service unavailable while loading vote details:",
          ledgerError,
        );
      }
  ...
  const displayedElections =
    currentView === "home" ? getActiveElections() : getHistoryElections();
  ...
  const selectedElectionResults = selectedElection
    ? (electionResults.get(selectedElection.id) ?? [])
    : [];
              ...
              <View style={styles.dialogHeader}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.dialogTitle}>
                    {selectedElection?.title ?? "Election"}
                  </Text>
              ...
              >
                <View style={styles.dialogMetaRow}>
                  ...
                  <Text style={styles.dialogDates}>
                    📅{" "}
                    {selectedElection
                      ? `${formatDate(selectedElection.starts_at)} — ${formatDate(selectedElection.ends_at)}`
                      : ""}
                  </Text>
                </View>
                ...
                {showVotingPanel ? (
                  <>
                    ...
                    {!registryStatus ? (
                      <Text style={styles.infoText}>
                        You are not registered for this election, so voting is
                        disabled.
                      </Text>
                    ) : null}
                    {candidates.length > 0 ? (
                  ...
                  <>
                    <View style={styles.voteDetailsContainer}>
                      ...
                      selectedElectionResults.length > 0 ? (
                        selectedElectionResults.map((result) => (
                          <View
                            key={result.candidateId}
                            style={styles.voteDetailRow}
                          >
                            <View style={{ flex: 1 }}>
                              <Text style={styles.voteDetailLabel}>
                      ...
                      {selectedVoteDetails ? (
                        <>
                          ...
                          <View style={styles.voteDetailRow}>
                            ...
                            <Text style={styles.voteDetailValue}>
                              {selectedVoteDetails.candidate}
                            </Text>
                          </View>
                        </>
                      ) : (
                        <Text style={styles.infoText}>
                          Your vote receipt is not available in this session.
                        </Text>
                      )}
                    </View>
                    ...
                    >
                      ...
                      <View style={styles.electionActionRow}>
                        {badgeStatus === "active" ? (
                          ...
                          >
                            <Text style={styles.electionPrimaryText}>
                              Cast Vote
                            </Text>
                          </Pressable>
                        ) : null}
                        ...
                        {badgeStatus === "closed" || badgeStatus === "voted" ? (
                          <Pressable
                            ...
                            onPress={() => void openElectionDialog(election)}
                          >
                            <Text style={styles.electionSecondaryText}>
                              See Results
                            </Text>
                          </Pressable>
                        ) : null}
