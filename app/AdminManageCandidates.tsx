...
export default function AdminManageCandidates({ isEmbedded }: { isEmbedded?: boolean } = {}) {
    ...
    try {
      ...
      const candidateEntries = await Promise.all(
        electionRows.map(async (election) => {
          const rows = await serviceFactory.adminService.getElectionCandidates(
            election.id,
          );
  ...
  const partiesByElection = useMemo(() => {
    ...
    elections.forEach((election) => {
      ...
      result[election.id] = Array.from(
        new Set([...fromCandidates, ...fromManual]),
      );
    });
  ...
  const getElectionName = (electionId: string) => {
    return (
      elections.find((election) => election.id === electionId)?.title ??
      "Unknown election"
    );
  };
  ...
  const getNextCandidateNumber = (electionId: string) => {
    ...
    const maxNumber = currentCandidates.reduce((maxValue, candidate) => {
      return candidate.candidate_number > maxValue
  ...
  const handleAddParty = () => {
    ...
    if (!electionId || !partyName) {
      Alert.alert(
        "Missing information",
        "Election and party name are required.",
      );
      return;
    }
    ...
    setManualPartiesByElection((prev) => {
      ...
      const alreadyExists = current.some(
        (party) => party.toLowerCase() === partyLabel.toLowerCase(),
      );
      if (alreadyExists) {
        return prev;
      }
  ...
  const handleAddCandidate = async () => {
    ...
    if (!electionId || !candidateName) {
      Alert.alert(
        "Missing information",
        "Candidate name and election are required.",
      );
      return;
    }
  ...
  return (
    <View style={styles.container}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalBoxLarge}>
            ...
            <Text style={styles.selectionLabel}>
              {getElectionName(addPartyElectionId)}
            </Text>
      ...
      <Modal
        ...
        animationType="fade"
        onRequestClose={closeAddCandidateModal}
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalBoxLarge}>
            <View style={styles.modalHeaderRow}>
              ...
              <Pressable
                style={styles.closeModalBtn}
                onPress={closeAddCandidateModal}
              >
                <Text style={styles.closeModalText}>x</Text>
              </Pressable>
            </View>
            ...
            <View style={styles.optionListWrap}>
              ...
              0 ? (
                <Text style={styles.helperText}>
                  No party saved for this election yet. You can still add
                  candidate without party.
                </Text>
              ) : (
                (partiesByElection[addCandidateElectionId] ?? []).map(
                  (party) => {
            ...
            <Text style={styles.inputLabel}>
              Or Enter Party/Affiliation (optional)
            </Text>
            <TextInput
              style={styles.input}
      ...
      <Modal
        transparent
        visible={showEditCandidateModal}
        animationType="fade"
        onRequestClose={closeEditCandidateModal}
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalBoxLarge}>
            <View style={styles.modalHeaderRow}>
              ...
              <Pressable
                style={styles.closeModalBtn}
                onPress={closeEditCandidateModal}
              >
                <Text style={styles.closeModalText}>x</Text>
              </Pressable>
            </View>
      ...
      <Modal
        transparent
        visible={showDeleteCandidateModal}
        animationType="fade"
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            ...
            <Text style={styles.modalMessage}>
              Delete {selectedCandidate?.display_name}? This action cannot be
              undone.
            </Text>
        ...
        <View style={styles.innerWrapper}>
          <View style={styles.titleSection}>
            ...
            <Text style={styles.pageSubtitle}>
              Candidates grouped by election with party and candidate controls.
            </Text>
          </View>
            ...
            {elections.length === 0 ? (
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyStateText}>
                  Create an election first, then add candidates.
                </Text>
              </View>
            ) : (
              elections.map((election) => {
                ...
                return (
                  <View key={election.id} style={styles.electionCard}>
                    <View
                      style={[
                        styles.electionCardHeader,
                        isMobile && styles.electionCardHeaderMobile,
                      ]}
                    >
                      ...
                      {candidateRows.length === 0 ? (
                        ...
                        candidateRows.map((candidate) => (
                          <View
                            ...
                            style={[
                              styles.candidateRow,
                              isMobile && styles.candidateRowMobile,
                            ]}
                          >
                            <View style={styles.candidateInfoCol}>
                              ...
                              <Text style={styles.candidatePartyText}>
                            ...
                            <View style={styles.rowButtons}>
                              ...
                              <Pressable
                                ...
                                onPress={() =>
                                  openDeleteCandidateModal(candidate)
                                }
                              >
                                <Text style={styles.deleteMiniBtnText}>
                                  Delete
                                </Text>
                              </Pressable>
                            </View>
...
const styles = StyleSheet.create({
  ...
  primaryActionBtn: {
    ...
    paddingVertical: 12,
    alignItems: "center",
    boxShadow: "0px 6px 14px rgba(46, 99, 227, 0.18)",
  },
  primaryActionText: {
    color: "#ffffff",
    fontSize: 14,
